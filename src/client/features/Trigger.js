...
class Trigger extends Component {
	...
	handleAllEventClick = (e) => {
	  ...
	  const filterDOM = dom => e.nativeEvent.path.filter(index =>
	    // e.preventDefault()
				 index.id == dom).length > 0;
	  ...
	  // 是否显示房间细节
	  if (e.nativeEvent.path.filter(index => index.className == 'toggleDetail').length > 0) {
	  ...
	  // 是否显示代码编辑器
	  if (e.nativeEvent.path.filter(index => index.className == 'codingClick').length > 0) {
	    allHold('showCodeEdit', !showCodeEdit);
	  } else if (
	    e.nativeEvent.path.filter(index => index.id == 'textArea').length > 0
	  ) {
	  ...
	  // 是否显示Emoji
	  if (e.nativeEvent.path.filter(index => index.id == 'emojiClick').length > 0) {
	    allHold('showEmoji', !showEmoji);
	  } else if (
	    e.nativeEvent.path.filter(index => index.id == 'emojiContainer').length > 0
	  ) {
	  ...
	  // 是否显示创建群input
	  if (e.nativeEvent.path.filter(index => index.id == 'addgroup').length > 0) {
	    allHold('is_show_create_group_input', true);
	  } else {
	    allHold('is_show_create_group_input', false);
	  }
	  // Switch Channel
	  if (e.nativeEvent.path.filter(index => index.className == 'roomList').length > 0) {
	    this.props.store.socket({
	      url: 'get currentRoomInfo',
	      name: e.nativeEvent.path.filter(index => index.className == 'roomList')[0].id,
	    });
	  }
	}
