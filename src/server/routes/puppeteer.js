...
exports.getCode = async (ctx, next) => {
  ...
  urls.map(async (e) => {
    ...
    if (e.name == 'gitter') {
      ...
      await button.click();
      const github = await iframe.$('.login-view__buttons > a.button-github--small');
      ...
      while (true) {
        ...
        // 获取可删除的按钮数组
        const reback = await frame.$$('#chat-container .isViewers .icon-ellipsis');
        await reback[0].click('middle');
        await sleep(2000);

        await frame.waitFor('.js-chat-action-delete');
        const deleteMsg = await frame.$$('.js-chat-action-delete');
    ...
    if (e.name == 'kitchen') {
      const login = await page.$('input[name=phone]');
      await login.type(phone);
      const button = await page.$('.login-reg-right');
      await button.click();
    }
